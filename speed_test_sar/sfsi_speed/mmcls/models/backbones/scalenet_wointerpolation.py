...
class BasicBlock(nn.Module):
    ...
    def forward_with_mask(self, x):
        ...
        out_conv1_fusion = mask_conv1 * out_conv1
        if VIS:
            visulize(mask_conv1_sigmoid, mask_conv1, out_conv1, None, out_conv1_fusion, name = 'conv1')

        out_conv2 = self.conv2(out_conv1_fusion)
        out_conv2 = self.norm2(out_conv2)

        if self.grid_conv:
            _conv2_interval = self.grid_interval
        ...
        out_conv2_fusion = mask_conv2 * out_conv2
        if VIS:
            visulize(mask_conv2_sigmoid, mask_conv2, out_conv2, None, out_conv2_fusion, name = 'conv2')
...
class ScaleNetWoInterpolation(nn.Module):
    """ResNet backbone.
    ...
    def __init__(self,
                 ...
                 develop=None):
        super(ScaleNetWoInterpolation, self).__init__()
    ...
    def train(self, mode=True):
        super(ScaleNetWoInterpolation, self).train(mode)
