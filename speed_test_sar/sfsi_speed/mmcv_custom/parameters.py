def parameters(net, base_lr):
    ...
    for m in net.modules():
        ...
        if hasattr(m, 'lr_mult'):
            ...
            lr_mult_param_groups[m.lr_mult] += list(m.parameters(recurse=False))
        else:
            default_lr_param_group += list(m.parameters(recurse=False))
    param_list = [{'params': default_lr_param_group}] + [{'params': p, 'lr': base_lr * lm} for lm, p in lr_mult_param_groups.items()]

    _total_length = len(list(net.parameters()))
    assert total_length == _total_length, '{} vs {}'.format(total_length, _total_length)
