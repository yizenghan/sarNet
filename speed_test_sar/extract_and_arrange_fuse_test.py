...
class Bottleneck_refine(nn.Module):
    ...
    def forward(self, x, mask, inference=True):
        ...
        if not inference:
            ...
            # assert(0==1)
            out = x * mask1
            # out = torch.where(mask1>0, x, torch.tensor(0.0))
            # t3 = time.time()
...
class maskGen(nn.Module):
    def __init__(self, groups=1, inplanes=64, mask_size=7):
        ...
        self.fc_gs.bias.data[1:2 * groups + 1:2] = 1.0
...
def _rearrange_features(feat, mask, residual):
    ...
    for n in range(l):
        ...
        residual[:, k * c_interval:(k + 1) * c_interval, h_1:h_1 + h_interval, w_1:w_1 + w_interval] += feat[pp][q, :, :, :] # + residual[:, k * c_interval:(k + 1) * c_interval, h_1:h_1 + h_interval, w_1:w_1 + w_interval]
        # t6 = time.time()
        # t65.append((t6 - t5) * 1000)
        q += 1
        kk = k
