    ...
    def forward(self, x, mask, inference=True):
        ...
        if not inference:
            ...
            for i in range(g):
                ...
                resh = rearrange(out_in[:,i*c_interval:(i + 1)*c_interval,:,:], 'b c (h1 h2) (w1 w2) -> (b h2 w2) c h1 w1',h1=h_interval,w1=w_interval)
                batch_number = resh.shape[0]

                a = np.flatnonzero(mask[:, i, :, :].cpu())
                ...
                out = F.conv2d(out, weight_g, padding=1)
                ...
                _, c_f, h_f, w_f = out.size()
                _, c, h, w = mask.size()

                all_zero = torch.zeros(batch_number,c_f,h_f,w_f)#.cuda(0)
                all_zero[a] = out
                x_after = rearrange(all_zero, '(b h2 w2) c h1 w1 -> b c (h1 h2) (w1 w2)',h2=h,w2=w)
                out_ls.append(x_after)
...
class sarModule(nn.Module):
    ...
    def forward(self, x, temperature=1e-8, inference=False):
        ...
        for i in range(len(self.base_module)):
            ...
            mask = torch.zeros((1, self.patch_groups, self.mask_size, self.mask_size))#.cuda(0)
