...
def main():
    ...
    isExists = os.path.exists(args.train_url)
    if not isExists:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print(f'models.{args.arch}.{args.arch_config}')
    model = eval(f'models.{args.arch}.{args.arch_config}')(args)
    model_arch_str = f'{args.arch}\n{args.arch_config}\n{str(model)}'
    with open(args.train_url + 'model_arch.txt', 'w') as output_file: 
    ...
    optimizer = torch.optim.SGD([{'params': [param for name, param in model.named_parameters() if 'gs' in name],
                            'lr': args.lrfact * args.lr, 'weight_decay': args.weight_decay},
                            {'params': [param for name, param in model.named_parameters() if 'gs' not in name],
    ...
    if args.eval:
        ...
        adaptive_inference(val_loader, model, criterion, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):  
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, args, filename=f'checkpoint.pth.tar')
...
def train(train_loader, model,criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output, gate_activation = model(input, temperature=args.temp)
        ...
        for act in gate_activation:
            ...
            loss_act_rate += torch.pow(args.target_rate-torch.mean(act), 2)
...
def adaptive_inference(val_loader, model,criterion, args):
    model.eval()
    num_params = params_count(model) / 1e6
    ...
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, act_rates,losses, losses_cls, losses_act, acc1, acc5, FLOPs],
        prefix='Test: ')
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            if args.gpu is not None:
                input = input.cuda(args.gpu, non_blocking=True)
            target = target.cuda(args.gpu, non_blocking=True)
            output, gate_activation, flops = model.module.forward_calc_flops(input, temperature=0.01)
            gate_activation = [a for a in gate_activation if a is not None]
            loss_cls = criterion(output, target)
            
            flops = torch.mean(flops/1e9)
...
def save_checkpoint(state, is_best, args, filename='checkpoint.pth.tar'):
    ...
    if is_best:
       shutil.copyfile(args.train_url + filename, args.train_url + 'model_best.pth.tar')
