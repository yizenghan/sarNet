...
class SARModule(nn.Module):
    def __init__(self, t,c,n,s, input_channel, output_channel,
        ...
        self.fuse_dw = ConvBNReLU(output_channel,output_channel,groups=output_channel)
        self.fuse_pw = ConvBNReLU(output_channel,output_channel,kernel_size=1)
    def forward(self,x, temperature=1e-6):
        ...
        output = self.fuse_dw(att*x_base + (1-att)*x_refine)
        output = self.fuse_pw(output)
    ...
    def forward_calc_flops(self, x, temperature=1e-6):
        ...
        flops += _flops
        x_base = F.interpolate(x_base, scale_factor = 2)
        output, _flops = self.fuse_dw.forward_calc_flops(att*x_base + (1-att)*x_refine)
        flops += _flops
        output, _flops = self.fuse_pw.forward_calc_flops(output)
        flops += _flops
...
def mobilenet_v2_width10_sar_fuseDW(args, **kwargs):
...
def mobilenet_v2_width13_sar_fuseDW(args, **kwargs):
...
def mobilenet_v2_width14_sar_fuseDW(args, **kwargs):
