...
class InvertedResidual_base(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, is_first=False):
        ...
        self.stride = 2 if is_first else 1
        # assert stride in [1, 2]
        ...
        self.dwc = ConvBNReLU(hidden_dim, hidden_dim, stride=self.stride, groups=hidden_dim)
...
class InvertedResidual_refine(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, patch_groups=1):
        ...
        if self.stride != 1 or inp != oup:
            ...
            nn.Conv2d(inp, oup, kernel_size=1, stride=self.stride, padding=0, bias=False, groups=patch_groups),
            nn.BatchNorm2d(oup)
            )
        else:
        ...
        # self.downsample = nn.AvgPool2d(3, stride=2, padding=1) if is_first else None
        # layers = []
        # print(self.stride, inp, oup, self.use_res_connect)
        self.expand_ratio = expand_ratio
        if expand_ratio != 1:
            # pw
            # layers.append(ConvBNReLU(inp, hidden_dim, kernel_size=1))
    ...
    def forward(self, x, mask):
        residual = x if self.res_connect is None else self.res_connect(x)
        g = mask.shape[1]
        m_h = mask.shape[2]
        ...
        mask3 = mask.clone()
        if g > 1:
            mask3 = mask3.unsqueeze(1).repeat(1,c_in//g,1,1,1).transpose(1,2).reshape(b,c_in,m_h,m_h)
        mask3 = F.interpolate(mask3, size = (h,w))
        x = x * mask3
        x = self.pwc(x)
    ...
    def forward_calc_flops(self, x, mask):
        flops = 0
        if self.res_connect is None:
            residual = x
        else:
            c_in = x.shape[1]
            residual = self.res_connect(x)
            flops += c_in * residual.shape[1] * residual.shape[2] * residual.shape[3] / self.patch_groups
        g = mask.shape[1]
        m_h = mask.shape[2]
...
class SARModule(nn.Module):
    def __init__(self, t,c,n,s, input_channel, output_channel,
        ...
        for i in range(n-1):
            ...
            blocks_base.append(InvertedResidual_base(input_channel_base, output_channel_base, stride=2, expand_ratio=t, is_first=is_first))
        ...
        self.fuse = InvertedResidual(output_channel,output_channel, stride=s, expand_ratio=t)
    def forward(self,x, temperature=1e-6):
        ...
        output = self.fuse(att*x_base + (1-att)*x_refine)
    ...
    def forward_calc_flops(self, x, temperature=1e-6):
        ...
        flops += _flops
        x_base = F.interpolate(x_base, scale_factor = 2)
        output, _flops = self.fuse.forward_calc_flops(att*x_base + (1-att)*x_refine)
        flops += _flops
...
class MobileNetV2(nn.Module):
    def __init__(self,
        ...
        t, c, _, s = inverted_residual_setting[0]
        output_channel = _make_divisible(c * width_mult, round_nearest)
        self.first_block = block(input_channel, output_channel, stride=s, expand_ratio=t)
...
def mobilenet_v2_width10_sar_res_3mask_bl(args, **kwargs):
...
def mobilenet_v2_width13_sar_res_3mask_bl(args, **kwargs):
...
def mobilenet_v2_width14_sar_res_3mask_bl(args, **kwargs):
