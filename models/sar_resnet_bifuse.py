...
class sarModule(nn.Module):
    def __init__(self, block_base, block_refine, in_channels, out_channels, blocks, stride, groups=1,mask_size=7, alpha=1):
        ...
        self.b2r = nn.Upsample(scale_factor=2, mode='nearest')
        self.r2b = nn.AvgPool2d(3, stride=2, padding=1)
    ...
    def forward(self, x, temperature=1e-8, inference=False):
        ...
        for i in range(len(self.base_module)):
            ...
            if i < len(self.base_module)-1:
                x_b2r = self.b2r(x_base)
                x_r2b = self.r2b(x_refine)
                x_base += x_r2b
                x_refine += x_b2r
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        sar_res = sar_resnet_bifuse(depth=50, patch_groups=1, width=1, alpha=2)
        print(sar_res)
        # y = sar_res()
        sar_res.eval()
