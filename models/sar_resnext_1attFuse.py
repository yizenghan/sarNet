...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, last_relu=True,patch_groups=1, 
        ...
        self.conv2 = nn.Conv2d(planes // 2, planes // 2, kernel_size=3, stride=stride, groups=32, 
        ...
        # print(inplanes, planes // 2, planes)
...
class Bottleneck_refine(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, last_relu=True,patch_groups=1, base_scale=2, is_first = True, groups_3x3=32):
        ...
        self.conv2 = nn.Conv2d(planes // 2, planes // 2, kernel_size=3, stride=stride, groups=32, 
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes // 2)
        self.conv3 = nn.Conv2d(planes // 2, planes, kernel_size=1, bias=False,groups=patch_groups)
        self.bn3 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
...
def sar_resnet_imgnet_alphaBase(depth, num_classes=1000, patch_groups=1, mask_size=7, width=1.0, alpha=1, beta=1, base_scale=2,use_stem=False):
    ...
    block = Bottleneck
    block_refine = Bottleneck_refine
...
def sar_resnext50_32x4_1attFuse(args):
