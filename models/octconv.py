...
class OctaveConv(nn.Module):
    ...
    def forward_calc_flops(self, x):
        ...
        x_h = self.downsample(x_h) if self.stride == 2 else x_h
        if self.stride == 2:
            flops += x_h.shape[1] * x_h.shape[2] * x_h.shape[3] * 4

        c_in = x_h.shape[1] 
        x_h2h = self.conv_h2h(x_h)
        flops += c_in * x_h2h.shape[1] * x_h2h.shape[2] * x_h2h.shape[3] * self.conv_h2h.weight.shape[2] * self.conv_h2h.weight.shape[3]
        if self.alpha_out > 0 and not self.is_dw:
            x_h2l = self.downsample(x_h)
            flops += x_h2l.shape[1] * x_h2l.shape[2] * x_h2l.shape[3] * 4
            c_in = x_h2l.shape[1]
            x_h2l = self.conv_h2l(x_h2l)
            flops += c_in * x_h2l.shape[1] * x_h2l.shape[2] * x_h2l.shape[3] * self.conv_h2l.weight.shape[2] * self.conv_h2l.weight.shape[3]
        else:
            x_h2l = None
        ...
        if x_l is not None:
            x_l2l = self.downsample(x_l) if self.stride == 2 else x_l
            if self.stride == 2:
                flops += x_l2l.shape[1] * x_l2l.shape[2] * x_l2l.shape[3]
            if self.alpha_out > 0:
                c_in =  x_l2l.shape[1]
                x_l2l = self.conv_l2l(x_l2l) 
                flops += c_in * x_l2l.shape[1]* x_l2l.shape[2] * x_l2l.shape[3] * self.conv_l2l.weight.shape[2] * self.conv_l2l.weight.shape[3]
            else:
                x_l2l =  None
            if self.is_dw:
                return x_h2h, x_l2l, flops
            else:
                c_in = x_l.shape[1]
                x_l2h = self.conv_l2h(x_l)
                flops += c_in * x_l2h.shape[1] * x_l2h.shape[2] * x_l2h.shape[3] * self.conv_l2h.weight.shape[2] * self.conv_l2h.weight.shape[3]
                x_l2h = self.upsample(x_l2h) if self.stride == 1 else x_l2h
                x_h = x_l2h + x_h2h
                x_l = x_h2l + x_l2l if x_h2l is not None and x_l2l is not None else None
                return x_h, x_l, flops
        else:
            return x_h2h, x_h2l, flops
...
class Conv_BN(nn.Module):
    ...
    def forward_calc_flops(self, x):
        x_h, x_l, flops = self.conv.forward_calc_flops(x)
        x_h = self.bn_h(x_h)
        x_l = self.bn_l(x_l) if x_l is not None else None
        return x_h, x_l, flops
