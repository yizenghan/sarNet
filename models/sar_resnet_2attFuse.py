...
class sarModule(nn.Module):
    def __init__(self, block_base, block_refine, in_channels, out_channels, blocks, stride, groups=1,mask_size=7, alpha=1,beta=1, base_scale=2):
        ...
        self.att_gen_base = AttentionLayer(channel=out_channels, reduction=16) 
        self.att_gen_refine = AttentionLayer(channel=out_channels, reduction=16) 
    ...
    def forward(self, x, temperature=1e-8, inference=False):
        ...
        att_base = self.att_gen_base(x_base)
        att_refine = self.att_gen_refine(x_refine)
        x_base = F.interpolate(x_base, size = (h,w))
        out = self.relu(att_base*x_base + att_refine*x_refine)
    ...
    def forward_calc_flops(self, x, temperature=1e-8, inference=False):
        ...
        att_base,_flops = self.att_gen_base.forward_calc_flops(x_base)
        flops += _flops
        att_refine,_flops = self.att_gen_refine.forward_calc_flops(x_refine)
        flops += _flops
        x_base = F.interpolate(x_base, size = (h,w))
        out = self.relu(att_base*x_base + att_refine*x_refine)
...
def sar_resnet34_2attFuse(args):
...
def sar_resnet50_2attFuse(args):
...
if __name__ == "__main__":
    ...
    args.beta = 1
    args.base_scale = 2
    sar_res = sar_resnet50_2attFuse(args)
    # print(sar_res)
