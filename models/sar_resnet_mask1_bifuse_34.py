...
__all__ = ['sar_resnet_mask1_bifuse_34']
...
class BasicBlock_refine(nn.Module):
    ...
    def forward_calc_flops(self, x, mask, inference=False):
        ...
        c_in = x.shape[1]
        out = self.conv1(x)
        flops += c_in * out.shape[1] * out.shape[2] * out.shape[3] * 9 / self.conv1.groups
        out = self.bn1(out)
        out = self.relu(out)
...
class sarModule(nn.Module):
    def __init__(self, block_base, block_refine, in_channels, out_channels, blocks, stride, groups=1, alpha=1.0):
        ...
        if alpha < 1:
            ...
            for _ in range(blocks - 1):
                conv_b2r.append(
                    nn.Sequential(
                        nn.Conv2d(int(out_channels * alpha), out_channels, kernel_size=1, padding=0, stride=1,
                                  bias=False),
                        nn.BatchNorm2d(out_channels)
                    ))
                conv_r2b.append(
                    nn.Sequential(
                        nn.Conv2d(out_channels, int(out_channels * alpha), kernel_size=1, padding=0, stride=1,
                                  bias=False),
    ...
    def forward_calc_flops(self, x):
        ...
        for i in range(len(self.base_module)):
            ...
            flops += _flops
            x_refine, _flops = self.refine_module[i].forward_calc_flops(x_refine) if i != 0 else self.refine_module[
                i].forward_calc_flops(x)
            flops += _flops
...
class sarResNet(nn.Module):
    ...
    def forward_calc_flops(self, x):
        ...
        c_in = x.shape[1]
        bx = self.b_conv0(x)
        flops += c_in * bx.shape[1] * bx.shape[2] * bx.shape[3] * self.b_conv0.weight.shape[2] * \
                 self.b_conv0.weight.shape[3]

        bx = self.bn_b0(bx)
        lx = self.l_conv0(x)
        flops += c_in * lx.shape[1] * lx.shape[2] * lx.shape[3] * self.l_conv0.weight.shape[2] * \
                 self.l_conv0.weight.shape[3]
        lx = self.bn_l0(lx)
        lx = self.relu(lx)

        c_in = lx.shape[1]
        lx = self.l_conv1(lx)
        flops += c_in * lx.shape[1] * lx.shape[2] * lx.shape[3] * self.l_conv1.weight.shape[2] * \
                 self.l_conv1.weight.shape[3]
        ...
        c_in = lx.shape[1]
        lx = self.l_conv2(lx)
        flops += c_in * lx.shape[1] * lx.shape[2] * lx.shape[3] * self.l_conv2.weight.shape[2] * \
                 self.l_conv2.weight.shape[3]
        lx = self.bn_l2(lx)
        x = self.relu(bx + lx)

        c_in = x.shape[1]
        x = self.bl_init(x)
        flops += c_in * x.shape[1] * x.shape[2] * x.shape[3] * self.bl_init.weight.shape[2] * self.bl_init.weight.shape[
            3]
        x = self.bn_bl_init(x)
        x = self.relu(x)
...
def sar_resnet_mask1_bifuse_34(depth, num_classes=1000, patch_groups=1, width=1.0, alpha=1):
    layers = {
        34: [3, 4, 6, 3],
        50: [3, 4, 6, 3],
        101: [4, 8, 18, 3],
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        sar_res = sar_resnet_mask1_bifuse_34(depth=34, patch_groups=1, width=1, alpha=1)
        # print(sar_res)
        # y = sar_res()
        sar_res.eval()
