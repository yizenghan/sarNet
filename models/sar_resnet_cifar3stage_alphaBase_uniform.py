...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, last_relu=True, patch_groups=1, 
                 mask_size=4, base_scale=2, is_first=False):
...
class BasicBlock_refine(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, last_relu=True, patch_groups=1, 
                mask_size=4, base_scale=2, is_first=True):
    ...
    def forward(self, x, inference=False):
        ...
        b,c,h,w = x.shape
        mask = torch.zeros((1, self.patch_groups, self.mask_size, self.mask_size), device = x.device)
        mask[:,:,::2,::2] = 1.0
        mask[:,:,1::2,1::2] = 1.0
        g = mask.shape[1]
        m_h = mask.shape[2]
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, last_relu=True, base_scale=2):
        ...
        if blocks == 1:
            layers.append(block(inplanes, planes, stride=stride, downsample=downsample,
                                patch_groups=self.patch_groups, base_scale=base_scale, 
                                mask_size=self.mask_size, is_first = False))
        else:
...
def sar_resnet32x2_un_cifar(args):
...
if __name__ == "__main__":
    ...
    args.base_scale = 2
    # with torch.no_grad():
    sar_res = sar_resnet32x2_un_cifar(args)
