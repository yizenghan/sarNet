...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, last_relu=True,patch_groups=1, 
        ...
        self.dilation_rate = base_scale
        self.padding_ = base_scale
...
class sarModule(nn.Module):
    def __init__(self, block_base, block_refine, in_channels, out_channels, blocks, stride, groups=1,mask_size=7, alpha=1,beta=1, base_scale=2):
        ...
        self.base_module = self._make_layer(block_base, in_channels, out_channels//alpha, blocks-1, stride=1, last_relu=base_last_relu, base_scale=base_scale)
        self.refine_module = self._make_layer(block_refine, in_channels, int(out_channels*beta) , blocks-1, stride=1, last_relu=refine_last_relu, base_scale=base_scale)
        self.alpha = alpha
        self.beta = beta
        if alpha != 1:
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, last_relu=True, base_scale=2):
        ...
        if blocks == 1:         # fuse, is not the first of a base branch
            ...
            layers.append(block(inplanes, planes, stride=1, downsample=downsample,patch_groups=self.patch_groups, 
    ...
    def forward_calc_flops(self, x, temperature=1e-8, inference=False):
        ...
        for i in range(len(self.base_module)):
            ...
            flops += _flops
            # print(x_base.shape, x_refine.shape)
        c = x_base.shape[1]
        _,_, h,w = x_refine.shape
        if self.alpha != 1:
            x_base = self.base_transform(x_base)
...
def sar_resnet50_1attFuse_dilated(args):
...
def sar_resnet101_1attFuse_dilated(args):
...
if __name__ == "__main__":
    ...
    args.base_scale = 2
    args.use_stem = False
    sar_res = sar_resnet50_1attFuse_dilated(args)
    # print(sar_res)
