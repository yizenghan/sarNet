...
class sarModule(nn.Module):
    def __init__(self, block_base, in_channels, out_channels, blocks, stride, groups=1,mask_size=4, alpha=1, beta=2, base_scale=2):
    ...
    def forward(self, x, temperature=1e-8, inference=False):
        ...
        x_base = F.interpolate(x_base, scale_factor=self.base_scale)
...
class sarResNet(nn.Module):
    def __init__(self, block_base, layers, num_classes=10, patch_groups=1, mask_size=4, width=1, alpha=1,beta=1, base_scale=2):
        # print(num_channels)
        self.inplanes = 16
        ...
        self.layer1 = sarModule(block_base, 16, int(16*width),
                               layers[0], stride=2, groups=patch_groups, mask_size=mask_size, alpha=alpha,beta=beta, base_scale=base_scale)
        self.layer2 = sarModule(block_base, int(16*width),
                                int(32*width), layers[1], stride=2, groups=patch_groups, mask_size=mask_size, alpha=alpha,beta=beta, base_scale=2)
        self.layer3 = sarModule(block_base, int(32*width), int(64*width),
...
def sar_resnet_alpha_cifar(depth, num_classes=100, patch_groups=2, mask_size=4, width=1, alpha=1,beta=1, base_scale=2):
    ...
    if depth == 16:
        width = 4
    model = sarResNet(block_base=BasicBlock, layers=layers,
...
def sar_resnet32x225_baseOnly_cifar(args):
    return sar_resnet_alpha_cifar(depth=32, num_classes=args.num_classes, patch_groups=args.patch_groups, mask_size=args.mask_size, width=2.25, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale)
def sar_resnet32x2_baseOnly_cifar(args):
...
if __name__ == "__main__":
    ...
    args.base_scale = 2
    # with torch.no_grad():
    sar_res = sar_resnet32x2_baseOnly_cifar(args)
    with torch.no_grad():
        x = torch.rand(1,3,32,32)
        sar_res.eval()
        cls_ops, cls_params = measure_model(sar_res, 32,32)
