...
class maskGen(nn.Module):
    def __init__(self, groups=1, inplanes=64, mask_size=7):
        ...
        self.fc = nn.Linear(inplanes, groups, bias=True)
    ...
    def forward(self, x, temperature=1.0):
        y = self.pool2(x).view(x.shape[0], x.shape[1])
        y = self.fc(y).view(x.shape[0], self.groups, 1, 1)
