...
class Bottleneck_refine(nn.Module):
    ...
    def forward(self, x, mask, inference=False):
        ...
        if mask is not None:
            ...
            if g > 1:
                mask2 = mask.unsqueeze(1).repeat(1, c_out // g, 1, 1, 1).transpose(1, 2).reshape(b, c_out, m_h,
                                                                                                    m_h)
            else:
                mask2 = mask.clone()
...
class sarModule(nn.Module):
    def __init__(self, block_refine, in_channels, out_channels, blocks, stride, groups=1,mask_size=7, alpha=1):
        ...
        for i in range(blocks - 1):
            if i != 0:
                ...
                mask_gen_list.append(maskGen(groups=groups, inplanes=in_channels, mask_size=mask_size))
        ...
        self.fusion = self._make_layer(block_refine, out_channels, out_channels, 1, stride=stride)
    ...
    def forward(self, x, temperature=1e-8, inference=False):
        ...
        for i in range(len(self.refine_module)):
            if i !=0:
                mask = self.mask_gen[i](x_refine, temperature=temperature)
                x_refine = self.refine_module[i](x_refine, mask, inference=False)
            else:
                mask = self.mask_gen[i](x, temperature=temperature)
                x_refine = self.refine_module[i](x, mask, inference=False)
            _masks.append(mask)
            
        if self.alpha > 1:
            x_refine = self.refine_transform(x_refine)
        out = self.relu(x_refine)
        out = self.fusion[0](out, mask=None, inference=False)
        return out, _masks
    ...
    def forward_calc_flops(self, x, temperature=1e-8, inference=False):
        ...
        for i in range(len(self.refine_module)):
            if i !=0:
                mask, _flops = self.mask_gen[i].forward_calc_flops(x_refine, temperature=temperature)
                flops += _flops
                x_refine, _flops = self.refine_module[i].forward_calc_flops(x_refine, mask, inference=False)
                flops += _flops
            else:
                mask, _flops = self.mask_gen[i].forward_calc_flops(x, temperature=temperature)
                flops += _flops
                x_refine, _flops = self.refine_module[i].forward_calc_flops(x, mask, inference=False)
                flops += _flops
        ...
        out = self.relu(x_refine)
        out, _flops = self.fusion[0].forward_calc_flops(out, mask=None, inference=False)
        flops += _flops
        return out, _masks, flops

class sarResNet(nn.Module):
    def __init__(self, block_refine, layers, num_classes=1000, patch_groups=1, mask_size=7, width=1.0, alpha=1):
        ...
        self.layer1 = sarModule(block_refine, num_channels[0], num_channels[0]*block_refine.expansion,
                               layers[0], stride=2, groups=patch_groups,mask_size=mask_size, alpha=alpha)
        self.layer2 = sarModule(block_refine, num_channels[0]*block_refine.expansion,
                               num_channels[1]*block_refine.expansion, layers[1], stride=2, groups=patch_groups,mask_size=mask_size, alpha=alpha)
        
        self.layer3 = sarModule(block_refine, num_channels[1]*block_refine.expansion,
                               num_channels[2]*block_refine.expansion, layers[2], stride=1, groups=patch_groups,mask_size=2, alpha=alpha)
        self.layer4 = self._make_layer(
            block_refine, num_channels[2]*block_refine.expansion, num_channels[3]*block_refine.expansion, layers[3], stride=2)
        self.gappool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(num_channels[3]*block_refine.expansion, num_classes)
        ...
        for m in self.modules():
            if isinstance(m, Bottleneck_refine):
    ...
    def forward(self, x, temperature=1.0, inference=False):
        ...
        for i in range(len(self.layer4)):
            x = self.layer4[i](x, mask=None, inference=False)
    ...
    def forward_calc_flops(self, x, temperature=1.0, inference=False):
        ...
        for i in range(len(self.layer4)):
            x, _flops = self.layer4[i].forward_calc_flops(x, mask=None, inference=False)
            flops += _flops
...
def sar_resnet_refineOnly(depth, num_classes=1000, patch_groups=1, mask_size=7, width=1.0, alpha=1):
    layers = {
        50: [3, 4, 6, 3],
        101: [4, 8, 18, 3],
        152: [5, 12, 30, 3]
    }[depth]
    model = sarResNet(block_refine=Bottleneck_refine, layers=layers,
...
if __name__ == "__main__":
    ...
    # print(sar_res)
    sar_res = sar_resnet_refineOnly(depth=50, patch_groups=2, width=1, alpha=1)
    # with torch.no_grad():
