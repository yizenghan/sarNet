...
class Bottleneck_refine_nomask(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, last_relu=True,patch_groups=1):
        super(Bottleneck_refine_nomask, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes // self.expansion, kernel_size=1, bias=False,groups=patch_groups)
        self.bn1 = nn.BatchNorm2d(planes // self.expansion)
        self.conv2 = nn.Conv2d(planes // self.expansion, planes // self.expansion, kernel_size=3, stride=stride,
                               padding=1, bias=False,groups=patch_groups)
...
class sarModule_nomask(nn.Module):
    def __init__(self, block_base, block_refine, in_channels, out_channels, blocks, stride, groups=1, alpha=1, beta=1):
        super(sarModule_nomask, self).__init__()

        self.relu = nn.ReLU(inplace=True)
        self.patch_groups = groups

        self.base_module = self._make_layer(block_base, in_channels, out_channels, blocks - 1, 2, last_relu=False)
        self.refine_module = self._make_layer(Bottleneck_refine_nomask, in_channels, out_channels // alpha, max(1, blocks // beta - 1), 1, last_relu=True)
        self.alpha = alpha
        if alpha > 1:
...
class sarResNet(nn.Module):
    def __init__(self, block_base, block_refine, layers, num_classes=1000, patch_groups=1, mask_size=7, width=1.0, alpha=1):
        ...
        self.layer1 = sarModule_nomask(block_base, block_refine, num_channels[0], num_channels[0]*block_base.expansion, 
        ...
        self.layer3 = sarModule_nomask(block_base, block_refine, num_channels[1]*block_base.expansion,
...
def sar_resnet_1stage(depth, num_classes=1000, patch_groups=1, mask_size=7, width=1.0, alpha=1):
    layers = {
        50: [3, 4, 6, 3],
        101: [4, 8, 18, 3],
        152: [5, 12, 30, 3]
