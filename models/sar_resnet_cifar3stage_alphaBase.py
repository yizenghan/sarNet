...
class sarModule(nn.Module):
    def __init__(self, block_base, block_refine, in_channels, out_channels, blocks, stride, groups=1,
        ...
        self.se_base = SELayer(channel=out_channels, reduction=16) if fuse_se else None
        self.se_refine = SELayer(channel=out_channels, reduction=16) if fuse_se else None
        self.fusion = self._make_layer(block_base, out_channels, out_channels, 1, stride=stride, base_scale=base_scale, is_fuse=True)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, last_relu=True, base_scale=2, is_fuse=False):
        ...
        if is_fuse:
            ...
            layers.append(block(inplanes, planes, stride, downsample,patch_groups=self.patch_groups,base_scale=base_scale, is_first = True))
...
class sarResNet(nn.Module):
    def __init__(self, block_base, block_refine, layers, num_classes=10, patch_groups=1, mask_size=4, width=1, alpha=1,beta=1, base_scale=2, fuse_se=False):
        # print(num_channels)
        self.inplanes = 16
        ...
        self.layer1 = sarModule(block_base, block_refine, 16, int(16*width),
                               layers[0], stride=2, groups=patch_groups, mask_size=mask_size, alpha=alpha,beta=beta, base_scale=base_scale, fuse_se=fuse_se)
        self.layer2 = sarModule(block_base, block_refine, int(16*width),
                                int(32*width), layers[1], stride=2, groups=patch_groups, mask_size=mask_size, alpha=alpha,beta=beta, base_scale=base_scale, fuse_se=fuse_se)
        self.layer3 = sarModule(block_base, block_refine, int(32*width), int(64*width),
                               layers[2], stride=1, groups=patch_groups, mask_size=2, alpha=alpha,beta=beta, base_scale=2, fuse_se=fuse_se)
        self.gappool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(int(64*width), num_classes)
...
def sar_resnet_alpha_cifar(depth, num_classes=100, patch_groups=2, mask_size=4, width=1, alpha=1,beta=1, base_scale=2, fuse_se=False):
    layers = {
        16: [2, 2, 2],
        20: [3, 3, 3],
        32: [5, 5, 5],
    ...
    model = sarResNet(block_base=BasicBlock, block_refine=BasicBlock_refine, layers=layers,
                    num_classes=num_classes, patch_groups=patch_groups, mask_size=mask_size, width=width, alpha=alpha, beta=beta, base_scale=base_scale, fuse_se=fuse_se)
    return model

def sar_resnet16x2_alphaBase_cifar(args):
    return sar_resnet_alpha_cifar(depth=16, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=False)

def sar_resnet20x1_5_alphaBase_cifar(args):
    return sar_resnet_alpha_cifar(depth=20, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=1.5, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=False)


def sar_resnet32_alphaBase_cifar(args):
    return sar_resnet_alpha_cifar(depth=32, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=1, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=False)

def sar_resnet32x2_alphaBase_cifar(args):
    return sar_resnet_alpha_cifar(depth=32, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=False)

def sar_resnet32x2_alphaBase_cifar_fuseSE(args):
    return sar_resnet_alpha_cifar(depth=32, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=True)

def sar_resnet20x2_alphaBase_cifar(args):
    return sar_resnet_alpha_cifar(depth=20, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=False)

def sar_resnet20x2_alphaBase_cifar_fuseSE(args):
    return sar_resnet_alpha_cifar(depth=20, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=True)

def sar_resnet56x2_alphaBase_cifar(args):
    return sar_resnet_alpha_cifar(depth=56, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=False)

def sar_resnet56x2_alphaBase_cifar_fuseSE(args):
    return sar_resnet_alpha_cifar(depth=56, num_classes=args.num_classes, patch_groups=args.patch_groups, 
                mask_size=args.mask_size, width=2, alpha=args.alpha, beta=args.beta, base_scale=args.base_scale, fuse_se=True)
...
if __name__ == "__main__":
    ...
    args.base_scale = 2
    # with torch.no_grad():
    sar_res = sar_resnet32_alphaBase_cifar(args)
