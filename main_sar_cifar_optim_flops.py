...
parser.add_argument('--lambda_act', default=10.0, type=float, metavar='M', help='momentum')
...
parser.add_argument('--optimize_rate_begin_epoch', default=0, type=int)
...
def main():
    ...
    if not args.train_on_cloud:
        if not os.path.exists(args.train_url):
            ...
            save_path = f'{args.train_url}{args.dataset}/{args.arch_config}/optimFLOPs_g{args.patch_groups}_a{args.alpha}b{args.beta}_s{args.base_scale}_t0_{str_t0}_target{str_ta}_optimizeFromEpoch{args.optimize_rate_begin_epoch}_lambda_{str_lambda}_dynamicRate{args.dynamic_rate}/'
...
def validate(val_loader, model, criterion, args, target_flops):
    ...
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, act_rates, FLOPs, losses, losses_cls, losses_flops, top1, top5],
        prefix='Test: ')
