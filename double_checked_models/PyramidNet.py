...
__all__ = ['pyramidnet50_a300', 'pyramidnet101_a360', 'pyramidnet200_a300']
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if residual_channel != shortcut_channel:
            ...
            padding = shortcut.new_zeros((batch_size, residual_channel - shortcut_channel,
...
class PyramidNet(nn.Module):
    ...
    def __init__(self, num_classes, depth, alpha, bottleneck=False, dropout=0.0):
    ...
    def pyramidal_make_layer(self, block, block_depth, stride=1):
        ...
        if stride != 1: # or self.inplanes != int(round(featuremap_dim_1st)) * block.outchannel_ratio:
            downsample = nn.AvgPool2d(kernel_size=2, stride=2, ceil_mode=True)
...
def pyramidnet50_a300(num_classes=1000, dropout=0.0):
    depth = 50
    alpha = 300
    model = PyramidNet(num_classes, depth, alpha, dropout=dropout)
    return model


def pyramidnet101_a360(num_classes=1000, dropout=0.0):
    depth = 101
    alpha = 360
    model = PyramidNet(num_classes, depth, alpha, dropout=dropout)
    return model
...
def pyramidnet200_a300(num_classes=1000, dropout=0.0):
    depth = 200
    alpha = 300
    model = PyramidNet(num_classes, depth, alpha, dropout=dropout)
    return model