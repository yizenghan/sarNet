...
__all__ = ['blresnext_model', 'blresnext50_32x4d_a2_b4', 'blresnext101_64x4d_a2_b4']
...
class bLResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, basewidth, cardinality, alpha, beta, num_classes=1000, dropout=0.0):
...
def blresnext50_32x4d_a2_b4(num_classes=1000, pretrained=False, dropout=0.0):
    depth = 50
    layers = [3, 4, 6, 3]
    cardinality = 32
    basewidth = 4
    ...
    model = bLResNeXt(Bottleneck, layers, basewidth, cardinality, alpha,
                      beta, num_classes, dropout=dropout)
    if pretrained:
        url = model_urls['blresnext-{}-{}x{}d-a{}-b{}'.format(depth, cardinality,
                                                              basewidth, alpha, beta)]
        checkpoint = torch.load(url)
...
def blresnext101_64x4d_a2_b4(num_classes=1000, pretrained=False, dropout=0.0):
    depth = 101
    layers = [4, 8, 18, 3]
    cardinality = 64
    basewidth = 4
