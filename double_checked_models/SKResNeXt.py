...
__all__ = ['sk_resnext50_32x4d', 'sk_resnext101_32x4d', 'sk_resnext152_32x4d']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.fc1 = conv1x1(width, reduction_planes)
        self.bn_fc1 = norm_layer(reduction_planes)
        self.fc2 = conv1x1(reduction_planes, 2 * width)
    ...
    def forward(self, x):
        ...
        u = u.reshape(-1, 2, self.width, 1, 1).contiguous()
        u = self.softmax(u)
        u1 = u1 * u[:, 0, :, :, :]
        u2 = u2 * u[:, 1, :, :, :]
        out = u1 + u2
...
def sk_resnext50_32x4d(num_classes, pretrained=False, progress=True, dropout=0.0, **kwargs):
...
def sk_resnext101_32x4d(num_classes, pretrained=False, progress=True, dropout=0.0, **kwargs):
...
def sk_resnext152_32x4d(num_classes, pretrained=False, progress=True, dropout=0.0, **kwargs):
    r"""ResNeXt-101 32x8d model from
    `"Aggregated Residual Transformation for Deep Neural Networks" <https://arxiv.org/pdf/1611.05431.pdf>`_
