...
def multi_thread_run(config='_sarResNet50_g1_blConfig', 
    ...
    train_url = f'{train_url_base}_preact_g{patch_groups}_alpha{alpha}_t0_{str_t0}_target{ta_str[-1]}_optimizeFromEpoch{optimize_rate_begin_epoch}_lambda_{str_lambda}_ls{use_ls}_amp{use_amp}_warmup{warmup}_dynamicRate{dynamic_rate}/'    
    cmd = f'CUDA_VISIBLE_DEVICES={gpu} python sarNet/main_sar.py  --no_train_on_cloud \
            --train_url {train_url} \
            --data_url {data_url} \
...
config1 = f'_pre50_g{args.patch_groups1}a{args.alpha1}s{args.base_scale1}_ls_warmup'
config2 = f'_pre50_g{args.patch_groups2}a{args.alpha2}s{args.base_scale2}_ls_warmup'
config3 = f'_pre50_g{args.patch_groups3}a{args.alpha3}s{args.base_scale3}_ls_warmup'
config4 = f'_pre50_g{args.patch_groups4}a{args.alpha4}s{args.base_scale4}_ls_warmup'
